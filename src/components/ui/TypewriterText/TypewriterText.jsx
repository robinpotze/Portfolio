...
export default function TypewriterText({
    lines,
    charSpeed = TYPEWRITER.CHAR_SPEED,
    rowPause = TYPEWRITER.ROW_PAUSE,
    delay = 0,
    className,
    rowClassName,
}) {
    ...
    useEffect(() => {
        ...
        if (currentChar < line.length) {
            timerRef.current = setTimeout(() => setCurrentChar((c) => c + 1), currentLine === 0 && currentChar === 0 ? delay + charSpeed : charSpeed);
        } else {
        ...
        <div className={className}>
            <AnimatePresence>
                {lines.map((line, i) => {
                    ...
                    return (
                        <motion.pre key={`${i}-${line}`} className={rowClassName} variants={rowVariants} initial="hidden" animate="visible">
                            {text}
