...
uniform float uTime;
uniform float uHasTransmission;
...
void main(){
    ...
    vec3 envReflection=max(textureCube(uEnvMap,reflectDir).rgb,vec3(.08,.1,.12));
    
    // Chromatic aberration refraction
    vec3 refractCol=vec3(0.);
    if(uHasTransmission>.5){
        ...
        refractCol=mix(vec3(.04,.05,.06),envReflection,.18+F*.32);
    }
    vec3 glassColor=refractCol;
    ...
    glassColor+=mix(glassColor,envReflection,F*.9)*(1.-F);
    
    // Brightness and color tint
    glassColor*=1.12;
    glassColor*=vec3(1.,1.05,1.1);
